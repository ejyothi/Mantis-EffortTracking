 ...
 * @access public
 */
function plugin_EffortTracking_hhmm_to_minutes( $p_hhmm) {
	sscanf($p_hhmm, "%d:%d", $hours, $minutes); 
	return $hours * 60 + $minutes;
}

function list_factors($p_bug_id)
{
	//TODO Need to identify project id from bug id
	$t_project_id = 0;
	
	$t_table = plugin_table('factors');
	$t_result_factors = db_query("SELECT * FROM $t_table WHERE project_id = $t_project_id ORDER BY factor_name ASC");
	$t_num_rows = db_num_rows($t_result_factors);
	$t_factors = array();
	for ( $i=0; $i < $t_num_rows; $i++ )
	{
		$t_row = db_fetch_array( $t_result_factors );
		$t_factors[] = array('id'=>$t_row['id'], 'factor_name'=>$t_row['factor_name']);
	}
	return $t_factors;
}
?>
